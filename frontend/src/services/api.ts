...
import { teamInvitationsAPI } from './teamInvitations';
...
interface User {
  ...
  is_admin: boolean;
  organization_id: number;
  organization_role: string;  // viewer, member, manager
  job_title?: string;
}
...
export const usersAPI = {
  ...
  getOrganizationUsers: async (): Promise<AxiosResponse<User[]>> => {
    console.log('DEBUG: Calling getOrganizationUsers');
    const token = localStorage.getItem('token');
    console.log('DEBUG: Token exists:', !!token);
    
    try {
      const response = await api.get('/users/organization-users');
      console.log('DEBUG: getOrganizationUsers success:', response.data);
      return response;
    } catch (error) {
      console.error('DEBUG: getOrganizationUsers error:', error);
      throw error;
    }
  },
};
...
export default {
  ...
  leads: {
    ...
    exportCSV(params?: {
      ...
      return leadsAPI.exportCSV(params);
    },
    downloadTemplate() {
      return leadsAPI.downloadTemplate();
    },
  ...
  teamInvitations: teamInvitationsAPI
};
