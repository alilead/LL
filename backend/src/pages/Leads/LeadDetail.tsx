...
export function LeadDetail() {
  ...
  const handleLinkedInConnect = () => {
    ...
    try {
      ...
      // Display a modal explaining the LinkedIn connection process
      if (window.confirm(
        "To send LinkedIn connection requests, you need:\n\n" +
        "1. A valid LinkedIn account\n" +
        "2. Connect your account to LeadLab\n\n" +
        "Do you want to proceed?"
      )) {
        // LinkedIn URL'den profil ID'sini çıkar - direkt URL'yi API'ye gönderiyoruz çünkü API içinde çıkarma işlemi yapılıyor
        toast.promise(
          linkedinAPI.sendConnection(linkedinUrl),
          {
            loading: 'Sending connection request...',
            success: 'LinkedIn connection request sent!',
            error: (err) => {
              console.error("LinkedIn connection error:", err);
              
              // Check if the error is related to LinkedIn authentication
              if (err.message?.includes("LinkedIn authentication") || 
                  err.message?.includes("connect your LinkedIn account") || 
                  err.message?.includes("LinkedIn token") ||
                  err.message?.includes("LinkedIn application credentials")) {
                
                // Ask user if they want to connect their LinkedIn account
                setTimeout(() => {
                  if (window.confirm("You need to connect your LinkedIn account first. Do you want to connect it now?")) {
                    // Redirect to LinkedIn authorization
                    linkedinAPI.authorize();
                  }
                }, 500);
                
                return "You need to connect your LinkedIn account first";
              }
              
              return `Failed to send connection request: ${err.message || 'Unknown error'}`;
            }
          }
        );
      }
      ...
      toast.error("An error occurred while trying to connect with LinkedIn");
    }
  };
  ...
  const analysisInfo = [
    ...
    { 
      label: 'Psychometrics (Personality Profiles)', 
      value: lead.psychometrics ? JSON.stringify(lead.psychometrics) : null,
      icon: <BarChart size={16} />,
      type: 'json' as const,
      fieldName: 'psychometrics' 
    },
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-12">
      ...
      >
        <div className="container mx-auto py-3 px-4 max-w-7xl flex justify-between items-center">
      ...
      <div className="container mx-auto py-6 px-4 max-w-7xl space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="bg-gradient-to-r from-primary/10 to-blue-50 p-6 md:p-8">
